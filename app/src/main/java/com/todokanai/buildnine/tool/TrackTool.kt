...
class TrackTool {
    ...
    fun mshuffle(){
        ...
        CoroutineScope(Dispatchers.IO).launch {
            MyDatabase.getInstance(myContext).roomNumberDao().deleteAll()
            MyDatabase.getInstance(myContext).roomNumberDao().insert(roomNumber)
        }
    ...
    fun mPrev(){
        if(mCurrent.value == 0){
            mCurrent.value = playListInfo.size-1
        } else{
            mCurrent.value = mCurrent.value!! - 1
        }
    }               // 이전곡 위치로 이동
    fun mNext(){
        if(mCurrent.value == playListInfo.size-1){
            mCurrent.value = 0
        } else {
            mCurrent.value = mCurrent.value!! + 1
        }
